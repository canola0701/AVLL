...
treeNode* LL_rotate(treeNode* parent) {//AVL트리회전
...
treeNode* RR_rotate(treeNode* parent) {//AVL트리 RR회전
...
treeNode* LR_rotate(treeNode* parent) {//AVL트리 LR회전
	treeNode* child = parent->left;
	parent->left = RR_rotate(child);
	return LL_rotate(parent);
}

treeNode* RL_rotate(treeNode* parent) {//AVL트리 RL회전
	treeNode* child = parent->right;
...
//서브 트리 높이를 이용해 균형 인수 BF 구하는 연산
int getBF(treeNode* p) {
	if (p == NULL)return 0;
	return getHeight(p->left) - getHeight(p->right);
}

//BF를 검사하여 불균형 발상한 경우, 회전 연산 호출
...
//AVL 트리에 노드를 삽입하는 연산 : 이진 탐색 연산처림 삽입 후, balance() 호출
